...
# Data Folder
#folder ='C:\\Users\\cryo\\Desktop\\George_CoolDown_12152017\\'
#Date File Name
filename = 'TantalumCoolDown_Data.txt'
...
with GracefulInterruptHandler() as h:
    while True :
        ...
        tempB = get_temp('B')
        time.sleep(Delay)
        print " %s   %f %f "%(time.strftime("%a, %d %b %Y %H:%M:%S "),tempA, tempB) #,pressure)
        datafile.write(" %s  %f %f  \n"%(time.strftime("%a, %d %b %Y %H:%M:%S "),tempA,tempB)) #,pressure))
        if h.interrupted:
